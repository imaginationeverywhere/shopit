  ...
  <div onClick={() => onSelect(color, isSelected)} className={`color-variant ${isSelected ? 'selected-color' : ''}`}>
    <div style={{
      backgroundColor: color
        ...
        {isSelected && <CheckIcon color={color === 'white' ? "#888" : '#ffffffdf'} />}
      </div>
  </div>
)

const ColorVariantComp = ({ variants, handleChange}) => {
  const colorArray  = [ "#b87145", "#f0c04a", "#333333", "#cc3333", "#3399cc", "#669933", "#f2719c", "#ebebeb", 'orange', 'violet', 'grey', 'pink', 'brown'];
  ...
  return (
    <div className="mt-1">
      ...
      {
        colorArray.map((ele, i) => <ColorSelect key={ele + i} color={ele} onSelect={onSelect} isSelected={variants && variants.includes(ele)} />)
      }
    </div>
    </div>
