...
(function ($) {
  ...
  var self = (Molla = {
    ...
    stickyHeader: function () {
      ...
      $(window).on("load scroll resize", function () {
        ...
        if ($(".sticky-header").length && $(window).width() >= desktop_width) {
          if ($(window).scrollTop() > height) {
            $(".sticky-header").each(function () {
              if (!$(this).hasClass("fixed")) {
                ...
                if (!$this.hasClass("fixed")) {
                  active_toggle = $this
                    .find(".dropdown-menu-wrapper.open-toggle")
                    .hasClass("show");
                }
                $this.addClass("fixed");
                $("body").hasClass("home") &&
            ...
            $("body").hasClass("home") &&
              $(".dropdown-menu-wrapper.open-toggle").each(function () {
                var $this = $(this);
                if (active_toggle) {
                  $this.closest(".sticky-header").hasClass("fixed") &&
                    $this.addClass("show");
                }
              });
          ...
          $last_sticky
            .parent()
            .siblings(".sticky-wrapper")
            .find(".sticky-header.fixed")
            .css("box-shadow", "none");
        }
      });
    },
    ...
    layoutInit: function ($container, selector) {
      // Init Isotope
      ...
      $(window).on("resize", function (e) {
        if (undefined == $container) {
          $("body")
            .find('[data-toggle="isotope"] .grid-item:not(.appear-animate)')
            .css("animation-fill-mode", "none")
            .css("-webkit-animation-fill-mode", "none");
          $("body")
            .find('[data-toggle="isotope"] .grid-item.appear-animate.animated')
            .css("animation-fill-mode", "none")
            .css("-webkit-animation-fill-mode", "none");
        } else {
          $container
            .find(".grid-item:not(.appear-animate)")
            .css("animation-fill-mode", "none")
            .css("-webkit-animation-fill-mode", "none");
          $container
            .find(".grid-item.appear-animate.animated")
            .css("animation-fill-mode", "none")
            .css("-webkit-animation-fill-mode", "none");
        }
      });
    ...
    countDown: function ($wrap) {
      if ($.fn.countdown) {
        ...
        $wrap.find(".deal-countdown").each(function () {
          if (
            typeof $(this).data("countdown_initialized") != "undefined" &&
            $(this).data("countdown_initialized")
          ) {
            return;
          }
          ...
          if ($target.length) {
            ...
            var scrolloffset =
              $(window).width() >= desktop_width
                ? $target.offset().top - sticky_header_height
                : $target.offset().top;
            $("html, body").animate(
              {
    ...
    fillEmptyDemos: function () {
      ...
      for (var i = 0; i < filters.length; i++) {
        ...
        for (; lackCount > 0; lackCount--) {
          ...
          $("#sectionDemos .grid").append(
            '<div class="grid-item ' +
              filter.slice(1) +
              '"><a class="demo-item coming-soon appear-animate" data-appear-options=' +
              JSON.stringify(opt) +
              "></a></div>"
          );
        }
    ...
    appearAnimate: function () {
      $(".appear-animate:not(.animated)").each(function () {
        $(this).appear(function () {
          ...
          $this.css(
            "animation-duration",
            (undefined == options.duration ? 1000 : options.duration) + "ms"
          );
          ...
          $this.addClass(
            (undefined == options.name ? "fadeIn" : options.name) + " animated"
          );
        });
      });
    },
    checkScrollSection: function () {
      ...
      if ($.isArray(section_ids)) {
        ...
        for (var i = 0; i < section_ids.length; i++) {
          if (cur_pos + sh + wh / 2 >= $(section_ids[i])[0].offsetTop) {
            // if scrollable section is below half of window height
            idx = i;
          } else {
            break;
          }
